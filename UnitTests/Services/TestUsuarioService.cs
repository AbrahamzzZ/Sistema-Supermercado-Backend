...
public class TestUsuarioService
{
    ...
    public async Task RegistrarUsuario_DeberiaFallar_SiClaveInvalida()
    {
        ...
        _mockValidator.Setup(v => v.ValidateAsync(It.IsAny<Usuario>(), default)).ReturnsAsync(new ValidationResult(new List<ValidationFailure> { new ValidationFailure("Clave", "La clave debe tener al menos 10 caracteres, incluyendo letras, n�meros y caracteres especiales.") }));
        var result = await _service.RegistrarUsuarioAsync(usuario);

        Assert.IsFalse(result.IsSuccess);
        Assert.AreEqual("La clave debe tener al menos 10 caracteres, incluyendo letras, n�meros y caracteres especiales.", result.Message);
    }
    ...
    public async Task RegistrarUsuario_DeberiaFallar_SiCorreoInvalido()
    {
        ...
        _mockValidator.Setup(v => v.ValidateAsync(It.IsAny<Usuario>(), default)).ReturnsAsync(new ValidationResult(new List<ValidationFailure> { new ValidationFailure("Correo Electronico", "El correo electr�nico no tiene un formato v�lido") }));
        var result = await _service.RegistrarUsuarioAsync(usuario);
